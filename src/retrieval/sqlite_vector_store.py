...
def _filtered_query_tokens(query: str) -> list[str]:
    tokens = [
        token
        for token in TOKEN_PATTERN.findall(query.lower())
        if token not in FTS_STOPWORDS
    ]
...
def _build_fts_query(query: str) -> str | None:
    plans = _build_fts_query_plans(query)
    return plans[0] if plans else None
 
 
def _build_fts_query_plans(query: str) -> list[str]:
    unique_tokens = _filtered_query_tokens(query)
    if not unique_tokens:
        return []
 
    quoted_tokens = [_quote_fts_token(token) for token in unique_tokens]
    plans: list[str] = []
    if len(unique_tokens) > 1:
        phrase_query = '"' + " ".join(token.replace(chr(34), "") for token in unique_tokens) + '"'
        and_query = " ".join(quoted_tokens)
        plans.extend([phrase_query, and_query])
 
    plans.append(" OR ".join(quoted_tokens))
...
class SQLiteReviewVectorStore:
    ...
    def search(
        ...
        *,
        ...
        fts_queries = _build_fts_query_plans(query)
        if not fts_queries:
            return []
        ...
        if source_filter:
            ...
            source_params.append(source_filter.lower())
 
        rows = []
        for fts_query in fts_queries:
            params: list[object] = [fts_query, *source_params, resolved_candidate_limit]
            try:
                rows = self.connection.execute(
                    f"""
                    SELECT r.id, r.document, r.metadata_json, r.embedding
                    FROM reviews_fts
        ...
        for _review_id, document, metadata_json, embedding_blob in rows:
